...
- **Env заменяет список профиля, а не сужает его.** `YT_ALLOWED_WRITE_ISSUES` полностью вытесняет значение из профиля — это сделано ради CI. Отсюда прямое следствие: тот, кто управляет окружением процесса, может этой переменной **ослабить** политику профиля, в отличие от `read_only` и `allowed_queues`, которые окружением не ослабляются. Если это неприемлемо, запускайте `yt` в окружении, где переменную выставить нельзя.
...
- **`YT_ALLOWED_WRITE_ISSUES`** заменяет список профиля целиком, то есть окружением область записи можно расширить (для `read_only` и `allowed_queues` env-override нет).
- **Boards, sprints, projects, fields, users** — сущности вне модели очередей: `board list`, `sprint list`, `project list` и справочники по `allowed_queues` не фильтруются и показывают названия объектов чужих очередей.
- **Ссылки на чужие задачи внутри разрешённой** — `issue get` (поля `parent`, `links`) и `issue changelog` могут содержать *ключи* задач других очередей, попавшие в саму задачу. Сами эти задачи прочитать нельзя (`issue get OPS-1` → exit 10), но факт существования ключа виден. Полные данные связанных задач (`link list`) отфильтрованы.
...
| `YT_ALLOWED_WRITE_ISSUES` | Список ключей задач через запятую: запись разрешена только в них (заменяет значение профиля). Значение без единого ключа (`","`) — `config_error` |
