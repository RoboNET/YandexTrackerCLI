...
Если федерация проворачивает `refresh_token` при каждом обновлении, CLI сохраняет новый токен в профиль сам — сессия живёт, пока ей пользуются. Когда сохранить не удалось (занят лок конфига, нет прав на запись), команда всё равно отрабатывает штатно, а в stderr уходит `{"warning":{"code":"refresh_token_not_saved",…}}`: в профиле остался мёртвый токен, и следующий запуск попросит `yt auth relogin`.
