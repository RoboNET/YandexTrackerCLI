...
Если федерация проворачивает `refresh_token` при каждом обновлении, CLI сохраняет новый токен в профиль сам — сессия живёт, пока ей пользуются. Когда сохранить не удалось, команда всё равно отрабатывает штатно, а в stderr уходит `{"warning":{"code":"refresh_token_not_saved",…}}`: в профиле остался мёртвый токен, и следующий запуск попросит `yt auth relogin`. Причины отказа: занят лок конфига, нет прав на запись, профиль за это время удалили, пересоздали под другую федерацию (или другой тип аутентификации) — либо его `refresh_token` уже обновил другой запуск `yt`. В последнем случае на диске остаётся более свежий токен: CLI намеренно не пишет свой поверх него.
...
- **Призыв.** Любое мутирующее обращение, в JSON-теле которого на любом уровне вложенности есть `summonees` или `maillistSummonees`, — эти поля рассылают письма людям и в списки рассылки. Проверяется фактическое тело запроса, а не разобранные флаги, поэтому через `--json-file`/`--json-stdin`, вложенный комментарий в теле перехода или payload `yt issue batch` их не протащить.
  ...
  ```
- **Мутации автоматизаций.** `POST`/`PUT`/`PATCH`/`DELETE` на `queues/{KEY}/triggers`, `queues/{KEY}/autoactions/{id}`, `queues/{KEY}/macros/{id}` — вся группа `yt automation`. Среди действий триггера есть HTTP-запрос наружу и отправка письма, а созданные триггер, автодействие и макрос переживают сессию. **Чтение (`GET`) остаётся разрешённым.**

Как это работает:

- Проверка стоит в HTTP-конвейере, до выхода в сеть; сегмент ресурса сверяется позиционно (`queues/{KEY}/{triggers|autoactions|macros}`), поэтому очередь с ключом `TRIGGERS` или `MACROS` под запрет не попадает. Сегменты пути декодируются до сравнения (`%74riggers` не обходит) и сравниваются без учёта регистра. `POST .../_search` считается чтением и проходит.
...
| `YT_EXTERNAL_EFFECTS` | Разбор строгий, как у `YT_READ_ONLY`. Значение `0`/`false`/`no`/`off` запрещает **инициировать рассылку и интеграции явно** (`summonees`/`maillistSummonees` в теле, мутации `triggers`/`autoactions`/`macros`); `1`/`true`/`yes`/`on` **не снимает** запрет профиля; любое другое значение — `config_error` (exit 9). См. [Запрет внешних эффектов](#запрет-внешних-эффектов) |
