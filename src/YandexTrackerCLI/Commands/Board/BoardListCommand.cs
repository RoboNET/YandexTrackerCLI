...
/// <summary>
/// Команда <c>yt board list</c>: выполняет <c>GET /v3/boards</c> с пагинацией
...
/// </summary>
public static class BoardListCommand
{
    /// <summary>
    /// Строит subcommand <c>list</c> для <c>yt board</c>.
    /// </summary>
    ...
    public static Command Build()
    {
        ...
        var cmd = new Command("list", "Список досок с пагинацией.");
        ...
        cmd.SetAction(async (parseResult, ct) =>
        {
            try
            {
                ...
                await using (var w = new Utf8JsonWriter(ms, new JsonWriterOptions { Indented = !Console.IsOutputRedirected }))
                {
                    w.WriteStartArray();
                    var count = 0;
                    await foreach (var el in ctx.Client.GetPagedAsync("boards", ct: ct))
                    {
                        el.WriteTo(w);
                        if (++count >= max)
                        {
