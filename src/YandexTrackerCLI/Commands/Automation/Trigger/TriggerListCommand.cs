...
/// <summary>
/// Команда <c>yt automation trigger list</c>: выполняет
/// <c>GET /v3/queues/{queue}/triggers/</c> с пагинацией через
/// <see cref="YandexTrackerCLI.Core.Api.TrackerClient.GetPagedAsync"/>
/// и печатает список триггеров очереди как единый JSON-массив на stdout.
/// Лимит записей задаётся через <c>--max</c>.
/// </summary>
public static class TriggerListCommand
{
    /// <summary>
    /// Строит subcommand <c>list</c> для группы <c>yt automation trigger</c>.
    /// </summary>
    ...
    public static Command Build()
    {
        ...
        var cmd = new Command("list", "Список триггеров очереди.");
            ...
            try
            {
                ...
                await using (var w = new Utf8JsonWriter(ms, new JsonWriterOptions { Indented = false }))
                {
                    ...
                    await foreach (var el in ctx.Client.GetPagedAsync(
                        $"queues/{Uri.EscapeDataString(queue)}/triggers/",
                        ct: ct))
                    {
                        el.WriteTo(w);
                        if (++count >= max)
