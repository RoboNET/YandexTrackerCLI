...
/// <summary>
/// Команда <c>yt automation trigger get &lt;id&gt; --queue &lt;key&gt;</c>:
/// выполняет <c>GET /v3/queues/{queue}/triggers/{id}</c> и печатает
/// JSON-описание триггера на stdout.
/// </summary>
public static class TriggerGetCommand
{
    /// <summary>
    /// Строит subcommand <c>get</c> для группы <c>yt automation trigger</c>.
    /// </summary>
    ...
    public static Command Build()
    {
        ...
        var cmd = new Command("get", "Получить триггер (GET /v3/queues/{q}/triggers/{id}).");
        ...
        cmd.SetAction(async (pr, ct) =>
        {
            try
            {
                ...
                var result = await ctx.Client.GetAsync(
                    $"queues/{Uri.EscapeDataString(queue)}/triggers/{Uri.EscapeDataString(id)}", ct);
