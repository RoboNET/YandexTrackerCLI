...
/// <summary>
/// Команда <c>yt automation trigger update &lt;id&gt;</c>: выполняет
/// <c>PATCH /v3/queues/{queue}/triggers/{id}</c>. Тело собирается из
...
/// </summary>
public static class TriggerUpdateCommand
{
    /// <summary>
    /// Строит subcommand <c>update</c> для группы <c>yt automation trigger</c>.
    /// </summary>
    ...
    public static Command Build()
    {
        ...
        var versionOpt = AutomationVersionOption.Create("триггера");

        var cmd = new Command("update", "Обновить триггер (PATCH /v3/queues/{q}/triggers/{id}).");
