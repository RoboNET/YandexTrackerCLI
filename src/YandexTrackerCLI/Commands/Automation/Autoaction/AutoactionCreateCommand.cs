...
/// <summary>
/// Команда <c>yt automation autoaction create</c>: выполняет
/// <c>POST /v3/queues/{queue}/autoactions/</c>. Тело собирается из источника
/// (<c>--json-file</c> / <c>--json-stdin</c>) и inline-флагов
/// (<c>--name</c>, <c>--active</c>, <c>--inactive</c>) через
/// <see cref="JsonBodyReader.ReadAndMerge"/>.
/// </summary>
public static class AutoactionCreateCommand
{
    /// <summary>
    /// Строит subcommand <c>create</c> для группы <c>yt automation autoaction</c>.
    /// </summary>
    /// <returns>Сконфигурированная <see cref="Command"/>.</returns>
    public static Command Build()
    {
        var queueOpt = new Option<string>("--queue") { Description = "Ключ очереди.", Required = true };
        var nameOpt = new Option<string?>("--name") { Description = "Имя автодействия (override)." };
        ...
        var cmd = new Command("create", "Создать автодействие (POST /v3/queues/{q}/autoactions/).");
        ...
        cmd.SetAction(async (pr, ct) =>
        {
            try
            {
                ...
                var result = await ctx.Client.PostJsonRawAsync(
                    $"queues/{Uri.EscapeDataString(queue)}/autoactions/", body, ct);
