...
/// <summary>
/// End-to-end тесты read-only guard'а на уровне CLI-команд группы <c>link</c>:
/// проверяют, что mutating-команды (<c>add</c>/<c>remove</c>) блокируются
/// <see cref="YandexTrackerCLI.Core.Http.ReadOnlyGuardHandler"/>, когда профиль
...
/// <c>link list</c> не блокируется. Мутируют глобальное state
/// (env + Console + AsyncLocal), поэтому последовательно.
/// </summary>
[NotInParallel("yt-cli-global-state")]
public sealed class LinkReadOnlyGuardTests
{
    /// <summary>
    ...
    /// <summary>
    /// <c>link add</c> в read-only-профиле — блокируется, HTTP не отправляется.
    /// </summary>
    [Test]
    public async Task LinkAdd_ReadOnlyProfile_Blocked_Exit3()
    {
        ...
        var exit = await env.Invoke(
            new[] { "link", "add", "DEV-1", "--to", "DEV-2", "--type", "relates" },
            sw,
            er);
    ...
    /// <summary>
    /// <c>link remove</c> в read-only-профиле — блокируется, HTTP не отправляется.
    /// </summary>
    [Test]
    public async Task LinkRemove_ReadOnlyProfile_Blocked_Exit3()
    {
        using var env = new TestEnv();
        env.SetConfig(ReadOnlyConfig);
        var inner = new TestHttpMessageHandler();
    ...
    /// <summary>
    /// GET-операция <c>link list</c> не mutating и должна проходить даже в
    /// read-only-профиле. Exit = 0.
    /// </summary>
    [Test]
    public async Task LinkList_ReadOnlyProfile_NotBlocked()
    {
        using var env = new TestEnv();
        env.SetConfig(ReadOnlyConfig);
