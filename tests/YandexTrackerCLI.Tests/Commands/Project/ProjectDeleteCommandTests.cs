...
/// <summary>
/// End-to-end тесты команды <c>yt project delete &lt;id&gt;</c>: ответ
/// <c>204 No Content</c> → success-маркер <c>{"deleted":"&lt;id&gt;"}</c>, и обработка
/// <c>404 Not Found</c>. Мутируют глобальное state (env + Console + AsyncLocal),
/// поэтому последовательно.
/// </summary>
[NotInParallel("yt-cli-global-state")]
public sealed class ProjectDeleteCommandTests
{
    /// <summary>
    /// <c>204 No Content</c>: exit 0, stdout содержит <c>{"deleted":"42"}</c>, запрос
    /// ушёл методом DELETE на <c>/entities/project/42</c>.
    /// </summary>
    [Test]
    ...
    public async Task Delete_404_ReturnsNotFound_Exit5()
    {
        ...
        var sw = new StringWriter();
        var er = new StringWriter();
        var exit = await env.Invoke(new[] { "project", "delete", "9999" }, sw, er);
