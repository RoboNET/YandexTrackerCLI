...
/// <summary>
/// End-to-end тесты команды <c>yt project get &lt;id&gt;</c>: успешное получение
/// проекта и обработка <c>404 Not Found</c>. Мутируют глобальное state
...
public sealed class ProjectGetCommandTests
{
    ...
    public async Task Get_ById_ReturnsRawJson()
    {
        ...
        var inner = new TestHttpMessageHandler().Push(_ =>
        {
            ...
            r.Content = new StringContent("""{"id":7,"name":"Alpha"}""", Encoding.UTF8, "application/json");
            return r;
        });
        ...
        var exit = await env.Invoke(new[] { "project", "get", "7" }, sw, er);

        await Assert.That(exit).IsEqualTo(0);
        using var doc = JsonDocument.Parse(sw.ToString());
        await Assert.That(doc.RootElement.GetProperty("id").GetInt32()).IsEqualTo(7);
        await Assert.That(doc.RootElement.GetProperty("name").GetString()).IsEqualTo("Alpha");
        await Assert.That(inner.Seen.Single().RequestUri!.AbsolutePath.EndsWith("/entities/project/7")).IsTrue();
    }
    ...
    public async Task Get_404_ReturnsNotFound_Exit5()
    {
        ...
        var sw = new StringWriter();
        var er = new StringWriter();
        var exit = await env.Invoke(new[] { "project", "get", "9999" }, sw, er);
