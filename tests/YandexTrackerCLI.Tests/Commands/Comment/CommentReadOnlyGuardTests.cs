...
/// comment-команды (<c>add</c>/<c>update</c>/<c>delete</c>) блокируются
...
/// <c>comment list</c>) не блокируются. Мутируют глобальное state (env + Console + AsyncLocal),
/// поэтому последовательно.
/// </summary>
[NotInParallel("yt-cli-global-state")]
public sealed class CommentReadOnlyGuardTests
{
    /// <summary>
    ...
    /// <summary>
    /// <c>comment add</c> в read-only-профиле — блокируется, HTTP не отправляется.
    /// </summary>
    [Test]
    public async Task CommentAdd_ReadOnlyProfile_Blocked_Exit3()
    {
    ...
    /// <summary>
    /// <c>comment update</c> в read-only-профиле — блокируется.
    /// </summary>
    [Test]
    public async Task CommentUpdate_ReadOnlyProfile_Blocked_Exit3()
    {
        using var env = new TestEnv();
        env.SetConfig(ReadOnlyConfig);
        ...
        var er = new StringWriter();
        var exit = await env.Invoke(
            new[] { "comment", "update", "DEV-1", "123", "--text", "upd" },
            sw,
            er);
        await AssertReadOnlyExit(exit, er);
    }
    ...
    /// <summary>
    /// <c>comment delete</c> в read-only-профиле — блокируется.
    /// </summary>
    [Test]
    public async Task CommentDelete_ReadOnlyProfile_Blocked_Exit3()
    {
        using var env = new TestEnv();
        env.SetConfig(ReadOnlyConfig);
        env.InnerHandler = new TestHttpMessageHandler();
    ...
    /// <summary>
    /// GET-операция <c>comment list</c> не mutating и должна проходить даже в
    /// read-only-профиле. Exit = 0.
    /// </summary>
    [Test]
    public async Task CommentList_ReadOnlyProfile_NotBlocked()
    {
        using var env = new TestEnv();
        env.SetConfig(ReadOnlyConfig);
