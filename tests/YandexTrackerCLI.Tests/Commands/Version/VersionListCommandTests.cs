...
/// <summary>
/// End-to-end тесты команды <c>yt version list --queue &lt;key&gt;</c>:
/// выполняет <c>GET /v3/queues/{queue}/versions</c> и печатает ответ как есть.
/// <c>--queue</c> — обязательный option. Мутируют глобальное state
/// (env + Console + AsyncLocal), поэтому последовательно.
/// </summary>
[NotInParallel("yt-cli-global-state")]
public sealed class VersionListCommandTests
{
    /// <summary>
    /// Успешный <c>200 OK</c> с массивом: exit 0, метод GET, URL заканчивается на
    /// <c>/queues/DEV/versions</c>, stdout — тот же массив.
    /// </summary>
    [Test]
    public async Task List_WithQueue_ReturnsArray()
    {
        ...
        await Assert.That(capturedPath!.EndsWith("/queues/DEV/versions", StringComparison.Ordinal)).IsTrue();
    ...
    public async Task List_MissingQueue_FailsBeforeHttp()
    {
        ...
        var sw = new StringWriter();
        var er = new StringWriter();
        var exit = await env.Invoke(new[] { "version", "list" }, sw, er);
