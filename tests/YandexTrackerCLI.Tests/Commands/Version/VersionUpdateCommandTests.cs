...
/// <summary>
/// End-to-end тесты команды <c>yt version update &lt;id&gt;</c>. Typed-режим (все поля
/// опциональны, даты валидируются как ISO 8601) и raw-режим (<c>--json-file</c>/<c>--json-stdin</c>).
/// Без источника данных → exit 2. Мутируют глобальное state (env + Console + AsyncLocal),
/// поэтому последовательно.
/// </summary>
[NotInParallel("yt-cli-global-state")]
public sealed class VersionUpdateCommandTests
{
    /// <summary>
    /// Typed-режим с <c>--name</c>/<c>--released</c>: метод PATCH, URL заканчивается на
    /// <c>/versions/42</c>, в теле — только указанные поля.
    /// </summary>
    [Test]
    public async Task Update_TypedFields_PatchesTargetedBody()
    {
        ...
        var exit = await env.Invoke(
            new[] { "version", "update", "42", "--name", "Renamed", "--released", "true" },
            sw,
            er);

        await Assert.That(exit).IsEqualTo(0);
        await Assert.That(capturedMethod).IsEqualTo(HttpMethod.Patch);
        await Assert.That(capturedPath!.EndsWith("/versions/42", StringComparison.Ordinal)).IsTrue();
    ...
    public async Task Update_JsonFile_PatchesRawBody()
    {
        ...
        var er = new StringWriter();
        var exit = await env.Invoke(
            new[] { "version", "update", "42", "--json-file", path },
            sw,
            er);
    ...
    public async Task Update_NothingToUpdate_Exit2()
    {
        ...
        var sw = new StringWriter();
        var er = new StringWriter();
        var exit = await env.Invoke(new[] { "version", "update", "42" }, sw, er);
