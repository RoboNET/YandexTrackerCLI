...
/// <summary>
/// End-to-end тесты команды <c>yt version delete &lt;id&gt;</c>: ответ <c>204 No Content</c>
...
public sealed class VersionDeleteCommandTests
{
    /// <summary>
    /// <c>204 No Content</c>: exit 0, stdout содержит <c>{"deleted":"42"}</c>, запрос
    /// ушёл методом DELETE на <c>/versions/42</c>.
    /// </summary>
    [Test]
    ...
    public async Task Delete_404_ReturnsNotFound_Exit5()
    {
        ...
        var sw = new StringWriter();
        var er = new StringWriter();
        var exit = await env.Invoke(new[] { "version", "delete", "9999" }, sw, er);
