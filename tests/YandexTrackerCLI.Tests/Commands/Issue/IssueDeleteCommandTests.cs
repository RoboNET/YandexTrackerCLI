...
/// <summary>
/// End-to-end тесты команды <c>yt issue delete &lt;key&gt;</c>: путь <c>DELETE /v3/issues/{key}</c>,
/// success-маркер при пустом теле и маппинг 404 → exit 5. Мутируют глобальное state
/// (env + Console + AsyncLocal), поэтому последовательно.
/// </summary>
[NotInParallel("yt-cli-global-state")]
public sealed class IssueDeleteCommandTests
{
    /// <summary>
    /// <c>204 No Content</c> — stdout содержит JSON <c>{"deleted":"DEV-1"}</c>,
    /// method = DELETE, path заканчивается на <c>/issues/DEV-1</c>, exit 0.
    /// </summary>
    [Test]
    public async Task Delete_204_PrintsDeletedEnvelope()
    {
        ...
        await Assert.That(path!.EndsWith("/issues/DEV-1", StringComparison.Ordinal)).IsTrue();

        using var doc = JsonDocument.Parse(sw.ToString());
        await Assert.That(doc.RootElement.GetProperty("deleted").GetString()).IsEqualTo("DEV-1");
    }
    ...
    public async Task Delete_404_ReturnsNotFound_Exit5()
    {
        ...
        var sw = new StringWriter();
        var er = new StringWriter();
        var exit = await env.Invoke(new[] { "issue", "delete", "DEV-NOPE" }, sw, er);
