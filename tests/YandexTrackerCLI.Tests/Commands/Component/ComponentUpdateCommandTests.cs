...
public sealed class ComponentUpdateCommandTests
{
    ...
    /// <summary>
    /// Merge: <c>--json-file</c> + scalar inline <c>--name</c> => override побеждает.
    /// </summary>
    [Test]
    public async Task Update_JsonFile_WithInlineOverride_Merges()
    {
        ...
        var path = Path.Combine(Path.GetTempPath(), "component-update-" + Guid.NewGuid().ToString("N") + ".json");
        await File.WriteAllTextAsync(path, """{"name":"old","description":"d"}""");
        ...
        var er = new StringWriter();
        var exit = await env.Invoke(
            new[] { "component", "update", "42", "--json-file", path, "--name", "new" }, sw, er);
