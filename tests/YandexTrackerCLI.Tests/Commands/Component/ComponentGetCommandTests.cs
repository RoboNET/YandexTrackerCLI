...
/// <summary>
/// End-to-end тесты команды <c>yt component get &lt;id&gt;</c>: успешное получение
/// компонента и обработка <c>404 Not Found</c>. Мутируют глобальное state
/// (env + Console + AsyncLocal), поэтому последовательно.
/// </summary>
[NotInParallel("yt-cli-global-state")]
public sealed class ComponentGetCommandTests
{
    /// <summary>
    ...
    public async Task Get_ById_ReturnsRawJson()
    {
        ...
        var exit = await env.Invoke(new[] { "component", "get", "7" }, sw, er);
        ...
        await Assert.That(inner.Seen.Single().RequestUri!.AbsolutePath.EndsWith("/components/7")).IsTrue();
    }
    ...
    public async Task Get_404_ReturnsNotFound_Exit5()
    {
        ...
        var sw = new StringWriter();
        var er = new StringWriter();
        var exit = await env.Invoke(new[] { "component", "get", "9999" }, sw, er);
