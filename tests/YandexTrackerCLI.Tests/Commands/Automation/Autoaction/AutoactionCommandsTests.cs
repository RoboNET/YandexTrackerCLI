...
public sealed class AutoactionCommandsTests
{
    ...
    /// <summary>
    /// Действие <c>Update</c> в read-формате (как его отдаёт GET) при обновлении
    /// конвертируется в write-формат: плоский словарь <c>&lt;fieldId&gt;: &lt;value&gt;</c>.
    /// </summary>
    [Test]
    public async Task Update_ReadFormatUpdateAction_PatchesWriteFormatBody()
    {
        ...
        var file = Path.Combine(env.Root, "aa-read-format.json");
        await File.WriteAllTextAsync(file, """
        {"name":"a","actions":[{"type":"Update","id":3,"update":[
          {"field":{"self":"https://api.tracker.yandex.net/v3/queues/DEV/localFields/frontier",
                    "id":"6a846c8ad035c8117efac652--frontier","display":"Рубеж"},
           "update":{"set":90}}]}]}
        """);
        ...
        await Assert.That(update.GetProperty("6a846c8ad035c8117efac652--frontier")
            .GetInt32()).IsEqualTo(90);
    }

    /// <summary>
    /// Read-формат действия <c>Update</c> конвертируется и при создании (POST).
    /// </summary>
    [Test]
    public async Task Create_ReadFormatUpdateAction_PostsWriteFormatBody()
    {
        using var env = new TestEnv();
        env.SetConfig(TestEnv.MinimalOAuthConfig);

        var file = Path.Combine(env.Root, "aa-create-read-format.json");
        await File.WriteAllTextAsync(file,
            """{"name":"a","actions":[{"type":"Update","update":[{"field":{"id":"frontier"},"update":{"set":90}}]}]}""");
        ...
        var exit = await env.Invoke(
            new[] { "automation", "autoaction", "create", "--queue", "DEV", "--json-file", file },
            new StringWriter(), new StringWriter());

        await Assert.That(exit).IsEqualTo(0);
        using var doc = JsonDocument.Parse(capturedBody!);
        await Assert.That(doc.RootElement.GetProperty("actions")[0]
            .GetProperty("update").GetProperty("frontier").GetInt32()).IsEqualTo(90);
    }
    ...
    /// </summary>
    [Test]
    public async Task Update_WithVersion_AddsVersionQueryParameter()
    {
        using var env = new TestEnv();
        env.SetConfig(TestEnv.MinimalOAuthConfig);
        ...
        await Assert.That(exit).IsEqualTo(0);
        await Assert.That(path!.EndsWith("/queues/DEV/autoactions/17", StringComparison.Ordinal)).IsTrue();
        await Assert.That(query).IsEqualTo("?version=5");
    }

    /// <summary>
    /// Без <c>--version</c> URL остаётся прежним — старое поведение не меняется.
    /// </summary>
    [Test]
    public async Task Update_WithoutVersion_HasNoVersionQueryParameter()
    {
        using var env = new TestEnv();
        env.SetConfig(TestEnv.MinimalOAuthConfig);
        ...
        var exit = await env.Invoke(
            new[] { "automation", "autoaction", "update", "17", "--queue", "DEV", "--name", "renamed" },
            new StringWriter(), new StringWriter());

        await Assert.That(exit).IsEqualTo(0);
        await Assert.That(query!.Contains("version", StringComparison.Ordinal)).IsFalse();
    }

    /// <summary>
    /// <c>activate --version N</c> добавляет <c>?version=N</c> в PATCH-URL.
    /// </summary>
    [Test]
    public async Task Activate_WithVersion_AddsVersionQueryParameter()
    {
        using var env = new TestEnv();
        env.SetConfig(TestEnv.MinimalOAuthConfig);
        ...
        var exit = await env.Invoke(
            new[] { "automation", "autoaction", "activate", "17", "--queue", "DEV", "--version", "7" },
            new StringWriter(), new StringWriter());

        await Assert.That(exit).IsEqualTo(0);
        await Assert.That(query).IsEqualTo("?version=7");
    }

    /// <summary>
    /// <c>deactivate --version N</c> добавляет <c>?version=N</c> в PATCH-URL,
    /// а без флага query остаётся пустым.
    /// </summary>
    [Test]
    public async Task Deactivate_VersionFlagControlsQuery()
    {
        using var env = new TestEnv();
        env.SetConfig(TestEnv.MinimalOAuthConfig);
        ...
        var withVersion = await env.Invoke(
            new[] { "automation", "autoaction", "deactivate", "17", "--queue", "DEV", "--version", "9" },
            new StringWriter(), new StringWriter());
        ...
        await Assert.That(queries[0]).IsEqualTo("?version=9");
        await Assert.That(queries[1].Contains("version", StringComparison.Ordinal)).IsFalse();
    }

    /// <summary>
    /// Поле <c>version</c> из тела (как его отдаёт GET) уходит в query-параметр,
    /// а из отправляемого JSON вырезается: API его в теле не принимает.
    /// </summary>
    [Test]
    public async Task Update_VersionInBody_MovesToQueryAndIsStripped()
    {
        ...
        var file = Path.Combine(env.Root, "aa-with-version.json");
        await File.WriteAllTextAsync(file, """{"id":17,"version":42,"name":"a"}""");
        ...
        var exit = await env.Invoke(
            new[] { "automation", "autoaction", "update", "17", "--queue", "DEV", "--json-file", file },
            new StringWriter(), new StringWriter());

        await Assert.That(exit).IsEqualTo(0);
        await Assert.That(query).IsEqualTo("?version=42");
        using var doc = JsonDocument.Parse(capturedBody!);
    ...
    /// <summary>
    /// Явный <c>--version</c> побеждает значение из тела, но само поле из тела
    /// всё равно вырезается.
    /// </summary>
    [Test]
    public async Task Update_ExplicitVersion_WinsOverBodyVersion()
    {
        using var env = new TestEnv();
        env.SetConfig(TestEnv.MinimalOAuthConfig);

        var file = Path.Combine(env.Root, "aa-with-version-2.json");
        await File.WriteAllTextAsync(file, """{"version":42,"name":"a"}""");
        ...
        var exit = await env.Invoke(
            new[] { "automation", "autoaction", "update", "17", "--queue", "DEV",
                    "--json-file", file, "--version", "99" },
            new StringWriter(), new StringWriter());

        await Assert.That(exit).IsEqualTo(0);
        await Assert.That(query).IsEqualTo("?version=99");
        using var doc = JsonDocument.Parse(capturedBody!);
        await Assert.That(doc.RootElement.TryGetProperty("version", out _)).IsFalse();
    }
}
