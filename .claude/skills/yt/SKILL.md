...
- **Файл конфига.** Граница держится, пока у вызывающего нет прямой записи в `~/.config/yandex-tracker/config.json` (или `$XDG_CONFIG_HOME/yandex-tracker/config.json`, либо путь из `YT_CONFIG_PATH`). Кто может править этот файл — снимет любую политику. Файл создаётся с правами `0600`; каталог держите `0700` и запускайте ограниченный процесс под пользователем без прав на запись туда и без возможности подменить `YT_CONFIG_PATH`. Рядом лежит `config.json.lock` (те же `0600`) — им `yt` сериализует одновременные правки конфига, чтобы параллельный запуск не откатил чужое ужесточение политики; если лок занят дольше 10 секунд, команда падает с `config_error` (exit 9), не записав ничего. Исключение — запись сразу после интерактивного входа (`yt auth login` с браузерным флоу, `yt auth relogin` и автоматический перелогин): там лок ждут до 60 секунд, потому что креденшелы на этот момент есть только в памяти; если и этого не хватило, сообщение об ошибке прямо говорит, что сессия получена, но не сохранена.
...
Федерация может выдавать новый `refresh_token` при каждом обновлении access-токена (старый в этот момент уже мёртв). `yt` подхватывает такой токен и сразу сохраняет его в профиль — специальных действий не требуется, конфиг остаётся рабочим для следующих запусков. Если сервер вернул тот же токен, конфиг не переписывается.

Когда сохранить не удалось, **текущая команда всё равно отрабатывает штатно** — access-токен на руках валиден, exit-код не меняется. Причины отказа: занят лок конфига, нет прав на запись, профиль удалили, профиль пересоздали под другую федерацию или другой тип аутентификации — либо его `refresh_token` уже успел обновить другой запуск `yt` (тогда на диске лежит более свежий токен, и `yt` намеренно не пишет свой поверх). В stderr при этом уходит отдельной строкой предупреждение:

```json
{"warning":{"code":"refresh_token_not_saved","message":"...","reason":"..."}}
```

Это значит, что в профиле остался уже недействительный `refresh_token`: следующий запуск, скорее всего, потребует `yt auth relogin --profile <name>`. Агенту: не считать это ошибкой команды (stdout валиден), но при разборе stderr отличать `warning` от `error` — у них разные корневые ключи.

Известное ограничение: два параллельных `yt` могут обновиться одновременно, сервер провернёт токен каждому, а в конфиг ляжет только один — второй получит предупреждение `refresh_token_not_saved` и свой токен на диск не запишет (затирать чужой более свежий он не станет). Следующий запуск при этом всё равно может уйти в перелогин. Лечится тем, чтобы не запускать параллельные `yt` на одном federated-профиле с протухшим access-токеном.
