...
- **Env заменяет, а не сужает.** `YT_ALLOWED_WRITE_ISSUES` **заменяет** список профиля целиком (сделано ради CI, где задача каждый раз своя). Отсюда важное ограничение: вызывающий, управляющий окружением, может этой переменной **ослабить** политику профиля — в отличие от `read_only` и `allowed_queues`, которые окружением не ослабляются. Если модель угроз этого не допускает, запускайте `yt` в окружении, где переменную выставить нельзя.
...
- `yt auth status` печатает действующий список в `allowed_write_issues` и его источник в `allowed_write_issues_source` (`profile` или `env`).
...
- **`YT_ALLOWED_WRITE_ISSUES`** заменяет список профиля целиком, то есть окружением область записи расширяется (у `read_only` и `allowed_queues` env-override нет).
...
| То же на один запуск | `YT_ALLOWED_WRITE_ISSUES=DEV-42 yt comment add DEV-42 --text "..."` |
