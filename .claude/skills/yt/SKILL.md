...
Когда сохранить не удалось (занят лок конфига, нет прав на запись), **текущая команда всё равно отрабатывает штатно** — access-токен на руках валиден, exit-код не меняется. В stderr при этом уходит отдельной строкой предупреждение:
...
Известное ограничение: два параллельных `yt` могут обновиться одновременно, сервер провернёт токен каждому, а в конфиг ляжет только последний — «проигравший» токен становится мёртвым и следующий запуск уйдёт в перелогин. Лечится тем, чтобы не запускать параллельные `yt` на одном federated-профиле с протухшим access-токеном.
